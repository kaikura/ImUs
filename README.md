...
# Scripts
The application starts with the execution of *comandantecheguevara.py* which calls *webcam-capture-v1.01.py*. The latter will permit the users to take photos of themselves by pressing the spacebar. The program is looped for a number of times equal to the number of users. At each iteration  in the cycle the user is asked to take a picture of himself pressing spacebar first camera. When the program collected a picture for each user it ends and returns the control to *comandantecheguevara.py*. The script now calls *purpose.py* che è il progamma che customizza la libreria morpher in modo da fondere le immagini assiem, trovarne la sedia per ogni copia tramite AVERAGER mettiam caso ABC AB AC BC e restit una globale ABC e crea il video con vidor Tutto questo vine ovlto tramite unappropriately directory tree. ALYSSAQ PER GLI ALTRI SCRIPT. The video is saved as an .avi ile in a customized directory. Then the control switches back to comandante that finally calls the procsessing script videoOSc.pde which is wotking on top of the genereated video. When the .pde is terminated we return one last time to comandate to terminate the whole process. On the second machine people counter is running, using the SECOND CAMERA as image acquisition sensor. People counter individua tramite una rete neurlae già trainata l'ggetto se è un trackableonject esseri umani dallalto e gli asocia un Id. L'ID è messo wul centroide calcolato dell' oggetto trovato. L'ID rimane trackata grazie al centroid tracker. Il prgramma calcola larea della figura decisa dalle ID e la manda tramite OSC alla patch in MaxMSP e al file file processing. 
