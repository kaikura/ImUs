...
1) A mobileNet, which uses depthwise separable convolution: The general idea behind depthwise separable convolution is to split convolution into two stages, a depthwise convolution followed by a pointwise convolution. Depthwise convolutions is used to apply a single filter per each input channel (input depth); it is very efficient with respect to standard convolution; however, it only filters input channels, it does not combine them to create new features, so an additional layer that computes a linear combination of the output of depthwise convolution is needed in order to generate these new high level features. Pointwise convolution is used for this purpose to create a linear combination of the output of the depthwise layer. This type of network (MobileNet) allows to actually reduce the number of parameters and computations and consequently increase the performance and the maximum processing fps with the same processing units. The MobileNet is a base network, i.e. provides high level features (normally for classification): in fact, if you put a fully connected layer at the end (on top) of this network, you have built a classifier. However, it's possible to remove fully connected layer and replace it with detection networks (SSD). 
2) an SSD built on top of the MobileNet (substituting the conventional fully connected layer for classification). SSD is a single shot detector network that uses the last convolutional layer on the base network (MobileNet) for detection task, exploiting the high level features built by the MobileNet in the first step. In the case of the object detector, the SSD discretizes the output space of bounding boxes (the bounding boxes are the rectangles in which the objects are predicted to be contained) into a set of default boxes over different aspect ratios and scales per feature map location (discretisation on possible objects with different characteristics/dimensions/position in the frame). At prediction time, the network generates scores for the presence of each object category in each default box and produces adjustments to the box to better match the object shape. Finally, after prediction time, the centroids of the boxes containing the objects are then computed, and they represent objects effective positions: an ID is then also assigned to each of these centroids.
